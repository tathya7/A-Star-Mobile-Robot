...
while check:
    x_goal = int(input("Enter the Destination X Position ({} to {}): ".format(500, 5000)))
    y_goal = int(input("Enter the Destination Y Position ({} to {}): ".format(0, 1000)))
...
print("Generating Path..., Algorithm Time is: ", (end-start))
...
cv2.circle(canvas, (int(x_start), int(y_start)), 20, (11, 92, 227), -1)
cv2.circle(canvas, (int(x_goal), int(y_goal)), 20, (0, 165, 255), -1)
...
class MoveRobot(Node):
    ...
    def __init__(self):
        ...
        self.i = 0
        self.odom_subs = self.create_subscription(Odometry, '/odom', self.odom_cb,10)
        self.cmd_vel_pub = self.create_publisher(Twist, '/cmd_vel', 10)




        self.vel_pub = self.create_publisher(Twist, '/cmd_vel',QoSProfile( depth = 10))
        self.timer = self.create_timer(0.6, self.vel_publisher)
    ...
    def odom_cb(self,msg):
        ...
        roll, pitch, yaw = euler_from_quaternion(data)

        self.x = x
        self.y = y
        self.th = np.deg2rad(yaw)
    ...
    def vel_publisher(self):
        ...
        if self.i < len(path):
            ...
            x_par, y_par, th_par, rpml, rpmr = path[self.i]
            print(x_par, y_par, th_par, rpml, rpmr)

            x_par = (x_par-x_start)/1000
            y_par = (y_start-y_par)/1000
            th_par = np.rad2deg(np.arctan2(y_par-self.y, x_par-self.x))


            th_diff = th_par - self.th
            th_diff = np.deg2rad(th_diff)

            dist_diff = np.sqrt((self.x-x_par)**2 + (self.y-y_par)**2)
            dist_diff = min(0.8,dist_diff)
            print(th_diff)

            self.vel = Twist()

            self.vel.linear.x = float(dist_diff)*0.5
            self.vel.angular.z = th_diff*0.05

            self.cmd_vel_pub.publish(self.vel)

            if dist((self.x, self.y),(x_par,y_par)) < 0.2:
                self.i = self.i + 1
